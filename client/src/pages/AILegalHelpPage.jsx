            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 p-4 sm:p-6 overflow-y-auto bg-slate-50/50 space-y-6">
                    {messages.map((msg, index) => (
                        <div key={index} className={`group flex gap-3 sm:gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="flex flex-col max-w-[85%] sm:max-w-[80%]">
                                <div className={`p-3 sm:p-4 rounded-2xl shadow-sm border border-slate-100 text-xs sm:text-sm w-full ${
                                    msg.role === 'user' 
                                    ...
                                    >
                                        ...
                                        {copyStatus.index === index && (
                                            <span className={`absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-2.5 py-1 text-[10px] font-semibold text-white rounded shadow-md pointer-events-none whitespace-nowrap animate-in fade-in duration-200 z-10 ${
                                                ...
                                                {copyStatus.type === 'success' ? (msg.role === 'user' ? 'Question copied!' : 'Answer copied!') : 'Unable to copy. Please try again.'}
                                                {/* Tooltip arrow */}
