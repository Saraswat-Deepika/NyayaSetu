...
const VoiceInputPage = () => {
    ...
    const handleCopy = (content, index) => {
        navigator.clipboard.writeText(content).then(() => {
            setCopyStatus({ index, type: 'success' });
            setTimeout(() => setCopyStatus({ index: null, type: null }), 2000);
        }).catch(err => {
            console.error("Failed to copy message:", err);
            setCopyStatus({ index, type: 'error' });
            setTimeout(() => setCopyStatus({ index: null, type: null }), 3000);
        });
    };
                            ...
                            <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs">
                                ...
                                >
                                    {SUPPORTED_LANGUAGES.map(lang => (
                                        <option key={lang.code} value={lang.code} className="bg-white text-slate-850">{lang.label}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
                                                            ...
                                                            {msg.queryId && (
                                                                <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 text-[10px] text-slate-400">
                                                                    ...
                                                                    <button 
                                                                        ...
                                                                        className="opacity-100 md:opacity-0 md:group-hover:opacity-100 focus:opacity-100 transition-opacity duration-200 p-1 rounded-lg border border-slate-200 bg-white text-slate-500 transition-all flex items-center gap-1.5 cursor-pointer focus:ring-2 focus:ring-blue-500 focus:outline-none relative"
                                                                        aria-label="Copy answer"
                                                                        title="Copy answer"
                                                                    >
                                                                        {copyStatus.index === index && copyStatus.type === 'success' ? (
                                                                            <>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                                        {locationResults.map((facility, idx) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex gap-2 mt-4 pt-2.5 border-t border-slate-100">
                                                    {facility.phone && (
                                                        <a 
